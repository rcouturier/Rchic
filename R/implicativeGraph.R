...
plotImplicativeGraph <- function(thres=99,value,cbvalue,color,list.selected.item) {
  ...
  for(i in 1:n) {
    from=listNodes[[i]][1]
    to=listNodes[[i]][2]
    if(as.numeric(as.character(rules[i,5]))>thres & as.numeric(list.selected.item[[which(list.variables==from)]]) & as.numeric(list.selected.item[[which(list.variables==to)]])) {
  ...
  for(i in 1:n) {
    ...
    from=rule[[1]][1]
    to=rule[[1]][2]
    if(as.numeric(as.character(rules[i,5]))>thres &  as.numeric(as.character(rules[i,1]))<as.numeric(as.character(rules[i,2])) & 
  ...
  if(length(nodes)>0) {
    ...
    for (i in 1:length(edges)) {
      ...
      for (j in 1:numSplines(edge)) {
        ...
        val=as.numeric(as.character(rules[paste(tail(edge),"->",head(edge)),5]))
        print(val)
        col="black"
