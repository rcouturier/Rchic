...
{                               /* --- insert a symbol */
  ...
  STE *nnew;                     /* new symbol table element */
  ...
  nnew = (STE*)malloc(sizeof(STE) +size +strlen(name) +1);
  if (!nnew) return NULL;        /* allocate memory for new symbol */
  nnew->name    = (char*)(nnew+1) +size;         /* and organize it */
  strcpy(nnew->name, name);      /* note the symbol name, */
  nnew->type    = type;          /* the symbol type, and the */
  nnew->level   = tab->level;    /* current visibility level */
  nnew->succ    = tab->bvec[i];  /* insert new symbol at the head */
  tab->bvec[i] = nnew++;         /* of the bucket list */
  #ifdef NIMAPFN                /* if name/identifier map, */
  tab->ids[tab->cnt] = (int*)nnew;
  *(int*)nnew = tab->cnt;        /* store symbol in ids. vector */
  #endif                        /* and set the symbol identifier */
  tab->cnt++;                   /* increment the symbol counter */
  return nnew;                   /* return pointer to data field */
}  /* st_insert() */
...
{                               /* --- sort name/identifier map */
  ...
  sort(nim->ids, nim->cnt, cmpfn, data);
